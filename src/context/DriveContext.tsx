...
const driveReducer = (state: DriveState, action: DriveAction): DriveState => {
  switch (action.type) {
    ...
    case "MERGE_FOLDERS":
      // Merge new folders with existing, removing duplicates
 ...
 const fetchFolders = useCallback(async (parentId?: string | null) => {
  try {
    ...
    if (response.data.success) {
      ...
      // Logic:
      // - If fetching root (null/undefined): Replace all folders
      // - If fetching subfolders: Merge to keep parent folder
      if (parentId === undefined || parentId === null) {
        dispatch({ type: 'SET_FOLDERS', payload: mappedFolders })
      } else {
        // Use the new MERGE_FOLDERS action
...
}, []) // No dependency needed // No state.folders dependency!

  const uploadFile = useCallback(
    async (file: File, folderId?: string | null) => {
      try {
        const targetFolderId =
          folderId !== null  ? folderId : getCurrentFolderId();
    ...
    async (name: string, parentId?: string | null) => {
      try {
        const targetParentId =
          parentId !== undefined ? parentId : getCurrentFolderId();
  ...
  const shareFile = useCallback(
    async (fileId: string, email: string, permission: "VIEW" | "EDIT") => {
      try {
  ...
  const fetchSharedFiles = useCallback(async () => {
    try {
      ...
      toast.error(
        error.response?.data?.message || "Failed to fetch shared files",
      );
      throw error;
    }
  }, []);
...
// In your DriveContext.tsx
const navigateToFolder = useCallback(async (folderId: string | null) => {
  // If folderId is null, go to root
  if (folderId === null) {
    dispatch({ type: 'SET_CURRENT_PATH', payload: [] })
  } else {
    // Find if the folder is already in the current path
    const existingIndex = state.currentPath.indexOf(folderId)
    
    if (existingIndex !== -1) {
      // If folder is already in path, truncate to that point
      dispatch({
        type: 'SET_CURRENT_PATH',
        payload: state.currentPath.slice(0, existingIndex + 1)
      })
    } else {
      // If it's a new folder, add it to the path
      dispatch({
        type: 'SET_CURRENT_PATH',
        payload: [...state.currentPath, folderId]
      })
    }
  }
